...
export default function EmpathyExamples() {
  ...
  useEffect(() => {
    if (scenario) {
      // シナリオに基づいて例を選択
      let examples = null;
      if (scenario.includes('仕事')) {
        examples = examplesData.empathy.work;
      } else if (scenario.includes('趣味')) {
        examples = examplesData.empathy.hobby;
      } else {
        // デフォルトは仕事の例
        examples = examplesData.empathy.work;
      }
        ...
        <div className="flex-1 flex flex-col items-center px-6 py-4">
          <div className="w-full max-w-md">
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-xl p-6 mb-6 shadow-sm border border-white/40">
              <h1 className="text-xl font-bold text-blue-600 mb-2">NGとOK例で学ぼう</h1>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => router.push('/conversation/empathy-new')}
                className="flex-1 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-xl px-6 py-3 hover:opacity-90 transition-all"
              >
                続けて練習
              </button>
