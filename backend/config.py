...
if not DATABASE_URL:
    ...
    if IS_PRODUCTION:
        ...
        # mysql-connector-pythonを使用し、SSL設定を明示的に追加
        DATABASE_URL = f"mysql+mysqlconnector://{MYSQL_USER}:{MYSQL_PASSWORD}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DATABASE}?charset=utf8mb4&ssl_disabled=False&ssl_verify_cert=False"
    else:
        # ローカルMySQL（開発環境）
        # Docker環境での接続判定
